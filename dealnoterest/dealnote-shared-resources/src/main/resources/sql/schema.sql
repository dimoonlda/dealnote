...
ALTER TABLE SERVICECLIENT ADD CONSTRAINT UNQ1_SERVICECLIENT_TYPECODE UNIQUE (TYPECODE);
...
ALTER TABLE DOCCLASS ADD CONSTRAINT PK_DOCCLASS PRIMARY KEY (ID);
CREATE SEQUENCE GEN_DOCCLASS_ID;
ALTER SEQUENCE GEN_DOCCLASS_ID RESTART WITH 100;

CREATE TABLE DOCCLASSDET (
    ID            INTEGER NOT NULL,
    DOCCLASSID    INTEGER NOT NULL,
    AGENTID       INTEGER NOT NULL,
    REGNUMNEXT    INTEGER,
    REGNUMPREFIX  VARCHAR(5)
);

ALTER TABLE DOCCLASSDET ADD CONSTRAINT PK_DOCCLASSDET PRIMARY KEY (ID);
ALTER TABLE DOCCLASSDET ADD CONSTRAINT FK_DOCCLASSDET_AGENT FOREIGN KEY (AGENTID) REFERENCES AGENTS (ID) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE DOCCLASSDET ADD CONSTRAINT FK_DOCCLASSDET_DOCCLASS FOREIGN KEY (DOCCLASSID) REFERENCES DOCCLASS (ID) ON DELETE CASCADE ON UPDATE CASCADE;

CREATE SEQUENCE GEN_DOCCLASSDET_ID;
ALTER SEQUENCE GEN_DOCCLASSDET_ID RESTART WITH 1000;